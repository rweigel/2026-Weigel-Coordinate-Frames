...
Currently, this is not possible due to the lack of standards. At present, the best that can be achieved is a citation to the definition in one of the papers above, but this is not sufficient for reproducibility -- in this work we show that how a given coordinate frame definition can be implemented in different ways, leading to different data providers giving different locations for a spacecraft and different software libraries giving different answers for \remove[r1]{a} the transformation a vector from one coordinate system to another.
...
\begin{itemize}
    ...
    \item \texttt{PySPEDAS} 1.7.28, which uses transformation code derived from \remove[]{the IDL version of \texttt{PySPEDAS}}\add[r1]{\texttt{SPEDAS}, which is written in IDL} \cite{Angelopoulos2024IDL}\remove[r1]{, which}\add{ \texttt{SPEDAS}} was from the \texttt{ROCOTLIB} Fortran \cite{ROCOTLIB} library, which was used for data from the Cluster spacecraft mission \cite{ClusterTools1993}.
...
\item \add[r2]{\texttt{GPS} -- Global Positioning System}
...
\item \add[r2]{UTC -- Coordinated Universal Time}
\end{itemize}
