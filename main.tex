...
\definechangesauthor[color=green]{r2}
...
Currently, this is not possible due to the lack of standards. At present, the best that can be achieved is a citation to the definition in one of the papers above, but this is not sufficient for reproducibility -- in this work we show that how a given coordinate frame definition can be implemented in different ways, leading to different data providers giving different locations for a spacecraft and different software libraries giving different answers for \deleted[id=r1]{a} the transformation a vector from one coordinate system to another.
...
\added[id=r2]{Finally, we report on an additional issue. We show that different data providers report different positions of spacecraft, and so even if both providers use the same coordinate system definition, different results may be obtained depending on the source of the spacecraft position. And although it is independent of the issue of ambiguity in the definitions of coordinate systems, it is related because the spacecraft position is commonly transformed into a quantity.}

\deleted[id=r1]{To quantify the impact of ambiguity in definitions and implementations, we give examples that demonstrate the reported positions for spacecraft in the same-named reference systems can significantly differ depending on the data source in section~\ref{sect:comparisons_ephemeris}, and that the source of these differences can be due to differences in the definition of a coordinate system and differences in how the positions were computed or derived. In section~\ref{sect:comparisons_software} we demonstrate that software that transforms a vector between two coordinate systems exhibits smaller, but possibly significant, differences.}
...
Terminology related to reference systems and frames also varies in other fields. For example, in SPICE (Spacecraft, Planet, Instrument, C-matrix, Events; \citeA{NAIFOverview2023}) states ``a reference frame (or simply `frame') is specified by an ordered set of three mutually orthogonal, possibly time-dependent, unit-length direction vectors''\deleted{; this definition does not include an origin}. In robotics, the term ``coordinate frame" refers to a set of three orthogonal axes and an origin relative to a different coordinate frame~\cite{Murray2017}. 
...
\begin{adjustwidth}{-1cm}{-1cm} % Left and Right margin changes
     ...
     \begin{subfigure}[b]{0.55\textwidth}
         ...
         \centering
         \includegraphics[width=\textwidth]{figures/MMS-2_SSCWeb-GSM_vs_CDAWeb-GSM.pdf}
     \end{subfigure}
     \par\bigskip\bigskip
     \begin{subfigure}[b]{0.55\textwidth}
     ...
     \begin{subfigure}[b]{0.55\textwidth}
         ...
         \centering
         \includegraphics[width=\textwidth]{figures/MMS-2_SSCWeb-GSM_vs_JPL-GSM.pdf}
     \end{subfigure}
...
\begin{itemize}
    ...
    \item \texttt{PySPEDAS} 1.7.28, which uses transformation code derived from \deleted[id=r1]{the IDL version of \texttt{PySPEDAS}}\added[id=r1]{\texttt{SPEDAS}, which is written in IDL} \cite{Angelopoulos2024IDL}\deleted[id=r1]{, which}\added[id=r1]{ \texttt{SPEDAS}} was from the \texttt{ROCOTLIB} Fortran \cite{ROCOTLIB} library, which was used for data from the Cluster spacecraft mission \cite{ClusterTools1993}.
    ...
    \item \texttt{SpiceyPy} 6.0.0 is a Python wrapper for the SPICE toolkit. Two versions of SPICE kernel files were used, indicated by \texttt{spicepy1} and \texttt{spicepy2}. \texttt{spicepy1} was used for reference frame transforms for the Van Allen Probes mission. \texttt{spicepy2} had an update to the \texttt{MAG} frame, which used a more recent version of the IGRF model \cite{Alken2021} to determine the Earth's magnetic dipole orientation. \added[id=r2]{(The IGRF model is maintained by the IAGA (IAGA, 2026) V-MOD (IAGA/V-MOD, 2026) Working Group.)}
    \item \texttt{SunPy} version 7.0.0, which extends the AstroPy coordinates framework to include space physics reference frames.
\end{itemize}

\deleted[id=r1]{The top subplots in Figure~\ref{fig:angles} show the angles between the Z axis of four coordinate frame pairs.} \added[id=r1]{In Figure~\ref{fig:angles}(a)-(d), the top subplots show the angles between the $Z$ axis of four coordinate frame pairs as computed by \texttt{geopack\_08\_dp}}. 

The angle between the $Z$ axis in frame $A$ and the $Z$ axis in frame $B$, $\angle (Z_A, Z_B)$, was computed by transforming the $Z$ axis in frame $A$ into frame $B$, and the angle was computed between this transformed axis and the $Z$ axis in frame $B$. 

The middle subplots in Figure~\ref{fig:angles}\added[id=r1]{(a)-(d)} show the difference of the angles between the $Z$ axes for each library with respect to that for \texttt{geopack\_08\_dp}. The choice of library used as the baseline for this comparison is arbitrary, and it should not be interpreted as implying that this library is preferred; in general, we are interested in the maximum differences among the libraries, which are shown in the lower panel of each subplot. The maximum absolute values \added[id=r1]{Figure~\ref{fig:angles}(a)-(d)} range from $4\cdot 10^{-9}$ degrees for \texttt{pyspedas} in Figure~\ref{fig:angles}(a) to $1\cdot 10^{-2}$ degrees for \texttt{spicepy1} in Figure~\ref{fig:angles}(c); the values for each library are given in the legend in the middle panel of each subplot. \added[id=r1]{From inspection of the source code for \texttt{geopack\_08\_dp} and \texttt{pyspedas}, we find that they appear to be different implementations of the algorithms in Russell (1971), which is consistent with the closeness of their transform results shown in Figure~3.}

%These values can be used as an uncertainty estimate when comparing data transformed using different software libraries.

The bottom panels of Figure~\ref{fig:angles}\addded[id=r1]{(a)-(d)} shows that across all libraries, the maximum absolute value of lines in the middle panel are ${\sim}0.01^\circ$ for \texttt{GEO}/\texttt{GSE}, ${\sim} 0.03^\circ$ for \texttt{GEO}/\texttt{GSM}, ${\sim}0.04^\circ$ for \texttt{GSE}/\texttt{GSM}, and ${\sim}0.03^\circ$ for \texttt{GSE}/\texttt{GSM}, all of which are comparable to the average angular change in Earth's dipole $Z$ axis of ${\sim}0.04^\circ$ per year.
...
\begin{enumerate}
    ...
    \item Mission--developed SPICE kernels for positions that are obtained from the mission team periodically and archived. The \texttt{GEI} position--related SPICE kernel information is used to generate the \texttt{J2000} spacecraft positions. Then, software developed as part of the International Solar-Terrestrial Physics program in the 1990s (ISTP; \citeA{ISTP2025}) is used to compute positions in different coordinate frames, including \texttt{GEI}, \texttt{GEO}, \texttt{GM}, \texttt{GSE}, \texttt{GSM}, and \texttt{SM}. \deleted[id=r2]{The ISTP software uses proprietary information needed for the reference frames, such as the position of the Sun, calculated by GSFC's Flight Dynamics Facility (FDF).  This approach is only used on ISTP--era missions, i.e., ACE, Polar, Wind, and Geotail.}\added[id=r2]{For ISTP--era missions, i.e., ACE, Polar, Wind, and Geotail, information needed for the reference frames, such as the position of the Sun, is calculated by GSFC's Flight Dynamics Facility (FDF) using proprietary software.}
...
There is an additional source of uncertainty --- the source of the positions. Missions have used direct \added[id=r2]{Doppler and }\deleted[id=r2]{GPS}\added[id=r2]{GNSS} measurements, commercial software such as STK (Systems Tool Kit), and data from facilities such as the GSFC Flight Dynamics Facility (FDF), the Deep Space Network (DSN), NAIF, and NORAD to produce the spacecraft positions for the mission. \deleted[id=r2]{In section~\ref{sect:comparisons_ephemeris}, we showed examples where the positions for two different data providers for the same spacecraft differed in a way that was unlikely to be due to differences in how the data providers implemented the reference frame.}

\added[id=r2]{Accurate sources of spacecraft position data are also important -- even if two researchers compute the location of a spacecraft in a given reference using the same transformation algorithm, their results can differ significantly if the source of the position data differs. In section~\ref{sect:comparisons_ephemeris}, we gave an example in which differences in reported spacecraft positions between two data providers were likely due to one using mission-provided positions and the other using two-line elements that did not capture a spacecraft maneuver. In general, the mission has access to positions derived from the most accurate tracking data, including direct Doppler and on-board GNSS measurements, which are more accurate than two-line element-derived positions and may include spacecraft maneuvers that are not resolved by two-line elements.}
...
\begin{itemize}
  ...
  Software should have a DOI for each major version, so the version used can be referenced rather than just a top-level DOI. Additional software citation recommendations are given in \citeA{Jackson2012}, \citeA{Smith2016}, and \deleted[id=r1]{\cite{Stall2023}}\added[id=r1]{\citeA{Stall2023}}.
...
\item \added[id=r2]{\texttt{GPS} -- Global Positioning System}
\item GSFC -- Goddard Space Flight Center (NASA center)
\item IAGA -- \added[id=r2]{International Association of Geomagnetism and Aeronomy}
\item IGRF -- International Geomagnetic Reference Field (geomagnetic model)\added[id=r2]{; maintained by the IAGA (IAGA, 2026) V-MOD (IAGA/V-MOD, 2026) Working Group.}
...
\item  \texttt{MAG} -- Geomagnetic (reference frame). Also referred to as Centered Dipole (\texttt{CD}) and \texttt{GM}.
...
\item \added[id=r2]{UTC -- Coordinated Universal Time}
\end{itemize}
