...
In the revised manuscript, changes made in response to the first reviewer's comments are in blue font and labeled with \textcolor{blue}{\texttt{r1}}; changes made in response to the second reviewer's comments are in brown font and labeled with \textcolor{brown}{\texttt{r2}}. Typographical errors corrected by the authors are in red font and labeled with \textcolor{red}{\texttt{a}}.
\\
}
\section{Reviewer 1}
...
\vspace{0.25in}
{\color{violet}
We have removed the referenced paragraph. See also the reply to Minor Comment~3 of Reviewer~2.
}
\vspace{0.15in}
...
Note that we have chosen not to attempt to explain all of the trends shown in the middle panels of Figure~3.  Although we generally understand the reasons, providing concrete justifications would require a new section that explains how each transform is implemented in detail. In this section, we are only trying to make the point that two different scientists performing the same transformation with different libraries will get different results.
}
\vspace{0.25in}
...
We have considered all the major concerns, and it is somewhat awkward that we have not made significant modifications to address some of them. When writing this manuscript, we have at various times attempted to provide additional details on historical context. The lead author felt he did not have sufficient experience and therefore reduced the level of detail on the evolution to a minimum to avoid inaccuracies and to keep the focus on the results and observations that justified the recommendations. However, we agree that additional historical context will be helpful to the reader and have made some additions in this regard. (Based on the comments in this review and other informal conversations, we believe there is a need for a paper that provides details on the historical development and the rationale for the decisions. We expect that such a paper will be valuable as the standards proposed in this manuscript are discussed. However, we have chosen for this paper to primarily document ``where we are'' versus ``how we got here''.)
\vspace{0.25in}
}
...
We are aware of, and have documented and compared, the formulas used for the GEI transformation to GEO (or any geocentric frame; some details are provided in our reply to minor comment 5 below). In the original draft, paragraph 5 states reasons why GEI reference frame transforms may differ. To address this comment, we have added a sentence to this paragraph that notes the historical reasons for this. 
...
\\
\\
``Hapgood (1992) uses equations from Doggett et al. (1990); and Fränz and Harper (2002) use equations from Siedelmann et al. (1992).''
\\
\\
to
\\
\\
``Fränz and Harper (2002) use equations from USNO (1992); Hapgood (1992) uses reduced-accuracy versions of these equations found in USNO (1992); both use the time J2000.0 as the epoch.''
\\
\\
Note that Fränz and Harper (2002) provide a formula for GMST that differs from that in USNO (1992) and from the referenced source of their equation, which was Meeus (2000). See also our reply to Minor Comment 5. (I am not able to find Meeus (2000); I am only able to find Meeus (1998), whose formula matches USNO 1992). 
\\

We have changed the in-line citation format and put the authors as editors in the bibliography.
}
\vspace{0.25in}
...
\vspace{0.25in}
{\color{violet}
In the submitted draft, we noted that Russell (1971) cites a Fortran program provided through private communication. We have modified the sentence to refer to Hapgood (1995), which provides more detail on the Russell (1971) \texttt{GEI}. See also our reply to Minor Comment 5.
}
\vspace{0.25in}
...
\vspace{0.25in}
{\color{violet}
We agree that there should be a discussion of astronomical time-related standards and have created a new subsection in section~6 that describes GMST and its relationship to reference frame standards.
...
\vspace{0.25in}
{\color{violet}
This was part of the motivation for the (now-removed) statement discussed in the reply to Reviewer 1's first major comment. 

We have removed ``and by SPICE to refer to a system with origin at the solar system barycenter (Acton (1997); NAIF (2025))''. Explaining this would require an account of how SPICE conceptualizes reference frames, which is unnecessary for the objectives of this paper. (In SPICE, one cannot simply say the reference frame is \texttt{J2000} --- one needs to define a frame with the orientation of \texttt{J2000} and an origin at the center of the Earth.)

The Acton~1997 reference was meant to refer to the SPICE project in the quote, but was misplaced and has been added to a later sentence in the bullet point text.
...
\vspace{0.25in}
{\color{violet}
This is a useful piece of information, and we have added this reference to both places where the MMS closest separation was mentioned in section~4.1.

We have decided not to comment on the claim on the NASA website that, in 2016, MMS achieved the closest separation of any multi-spacecraft mission of ${\sim}7$~km, while Escoubet et al. (2021) state that ${\sim}4$~km was achieved in 2013 by Cluster.
...
\vspace{0.25in}
{\color{violet}
We agree on the possible explanation for the large differences shown in the bottom panel of Figure 1a and have made initial attempts to explain the observed differences in all figures (including estimates based on the angular change of a \texttt{GEI} with respect to \texttt{J2K} as suggested by the reviewer). We have found this difficult because legacy software is used that is neither publicly available nor executable on modern operating systems. As a result, we have previously chosen not to speculate on the explanation. A confounding factor is that if the ``\texttt{GEI}'' shown in Figure~1a was in fact \texttt{J2K}, as suggested, then Figure~1b would show the same differences as Figure~1a.

We believe this is primarily a standards issue. The fact is that what is meant by \texttt{GEI} depends on the models used for the ecliptic plane, precession, and nutation. The standard needs to account for the fact that ``\texttt{GEI}`` should always be qualified. 
\href{https://github.com/spacepy/spacepy/blob/main/spacepy/ctrans/__init__.py}{SpacePy})
defines \texttt{ECI2000}, \texttt{ECITOD}, and \texttt{ECIMOD}. This is only part of what is needed --- their definitions also need to specify the options used to realize the frame, so that two people reading the definition can independently produce the same realization.

In reviewing the literature, we find that the problem has less to do with understanding the physics of planetary and satellite dynamics and more with the more difficult task of relating definitions to one another. My experience has been that researchers will find a reference that defines a coordinate frame and use it without awareness that another researcher using a different reference will not get the same result. Even if the researcher fully understands positional astronomy, they may be surprised that a frame transform matrix depends on the reference they use. For example, the widely used Tsyganeko Geopack library uses a GMST definition based on Russell 1971 in their calculation of \texttt{GEI}. Hapgood 1992 uses a more accurate, but different, GMST definition, which differs from Russell 1971 by 0.00131$^\circ$ on 2020-01-01T00:00:00Z. The \texttt{cxform} library, which implements the Hapgood 1992 definition of GMST, switched from the Hapgood 1992 definition to that in Franz and Harper (2002). Franz and Harper (2002) use a definition that differs from Russell (1971) by 0.000951$^\circ$. Franz and Harper (2002) cite Meeus (1998) for their GMST formula, but actually use a slightly different formula (but their GMST values match to four significant digits).
Finally, the USNO web service https://aa.usno.navy.mil/data/siderealtime gives values that differ by 0.0000940$^\circ$. 

Over time, more accurate equations are used for GMST. The result is that we have author-dependent definitions of \texttt{GEI} transforms to geocentric frames. Thus, we have a situation in which one could argue that these differences are not significant relative to uncertainties in spacecraft data, yet modified and improved equations are used for GMST.
...
\vspace{0.25in}
{\color{violet}
Indeed, in developing this work, we struggled with this and appear to have neglected to include it in our recommendations. Early on, the positional data from one data provider differed significantly due to an error in their metadata regarding the value of $R_E$ ($6.4\cdot 10^6$ was reported).
\\
\\
...
\vspace{0.25in}
{\color{violet}
The bibliography entry has been modified to have a DOI in place of ``TBD''. We have also added a reference in the Open Data section to the repository for the code that uses \texttt{hxform} for the analysis in this manuscript.}
\end{document}
