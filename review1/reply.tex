...
\\
{\color{violet}\noindent
We had previously disabled transforms involving \texttt{MAG} and PySPEDAS due to an implementation error (\url{https://github.com/spedas/pyspedas/issues/1207}). After this issue was resolved, we neglected to re-enable these transforms. We have updated Figure 3(d) to include the PySPEDAS result.
}
\\
\\
...
\\
{\color{violet}\noindent
In section 4.2 we noted that the PySPEDAS transformation code originated from ROCTOLIB (we have modified the text to improve the wording and make clear that \texttt{SPEDAS} is written in IDL).
\\
\\
...
\\
{\color{violet}\noindent
This was corrected, but the change is not annotated because the trackchanges package does not work for citation changes.
}
