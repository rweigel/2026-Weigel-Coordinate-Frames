...
from ephemeris_plot import plot
...
# Options for _compute_diffs() in ephemeris_plot.py
popts = {
  'R_E': R_E,
  'cos_warnings': False,
...
def _infos(satellite):
  ...
  if satellite not in known_satellites:
    raise ValueError(f"Unknown satellite: {satellite}. Must be one of {known_satellites}")
...
def _jpl(info, time, logging=False):
  ...
  to = info['start']
  tf = info['stop']
  cache_file = f"data/jpl/jpl-{info['spacecraft']}-{info['frame']}-{to}-{tf}.pkl"
...
def _other_compare(satellite, infos, popts):
  ...
  for info in infos['sscweb']:
    if info is not None and info.get('frame') == 'J2K':
      sscweb_j2k = _sscweb(info, logging=hapi_logging)
    if info is not None and info.get('frame') == 'GEI':
      sscweb_gei = _sscweb(info, logging=hapi_logging)

  plot(satellite, sscweb_j2k, sscweb_gei, popts)
...
def cli():
  ...
  def validate_satellites(satellites):
    for satellite in satellites:
      if satellite not in known_satellites:
        raise argparse.ArgumentTypeError(f"Invalid satellite: {satellite}. Must be one of {known_satellites}")
    return satellites
  ...
  parser.add_argument(
    ...
    help=f"Comma-separated list of external satellites. Possible values {known_satellites}. Default is all satellites.",
    default=known_satellites
  )
...
for satellite in args.satellites:
  ...
  for i in range(len(infos['sscweb'])): # Loop over frames
    ...
    sscweb = _sscweb(infos['sscweb'][i], logging=hapi_logging)
    cdaweb = _cdaweb(infos['cdaweb'][i], logging=hapi_logging)
    amda = _amda(infos['amda'][i], logging=hapi_logging)

    times = infos['sscweb'][i]['time']
    jpl = _jpl(infos['jpl'][i], times, logging=hapi_logging)
    _print_first_last_(sscweb, cdaweb, amda, jpl)

    sources = [sscweb, cdaweb, amda, jpl]
    results[frame] = sources
    ...
    for i, source1 in enumerate(sources):
      for j, source2 in enumerate(sources):
        ...
        plot(satellite, source1, source2, popts)

fname = 'data/ephemeris/ephemeris'
print(f"Writing ephemeris results to {fname}.{{json,pkl}}")
